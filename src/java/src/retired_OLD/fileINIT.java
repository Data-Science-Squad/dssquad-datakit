 ...
 * */
public class fileINIT { // DEFINE CLASS NAME
	...
	public static void writeFileName(String fileName, String line) {
		/*PRINTS LINE TO OUTPUT PATH */
    ...
    public static void logMessage(String line) { // USED TO WRITE TO LOG FILES
		...
		try (BufferedWriter bw = new BufferedWriter(new FileWriter(file, true))) {// OPEN WRITER TO LOG FILE PATH 
		    if(firstLine) bw.write(  DateRoutines.timestamp() + ":: "+  line); //FIRST LINE, WRITE TIME STAMP A SEPARATOR (::) AND LOG MESSAGE
		    else  bw.write("\r\n" + DateRoutines.timestamp() + ":: "+  line);	//NEW LINE, WRITE TIME STAMP A SEPARATOR (::) AND LOG MESSAGE
		} catch(Exception e) {//CATCH IO EXCEPTIONS
			e.printStackTrace(); // PRINT ERROR ON SYSTEM CONSOLE
	...
	public static boolean collectCrimeEntities() { // THIS WILL BREAKDOWN OUR INPUT CSV
		try(BufferedReader br = new BufferedReader(new FileReader(Config.inputFile.replace(".csv", "_initiate.csv")))) { // OPEN CRIME_INCIDENTS.CSV
			...
			while((line=br.readLine()) != null) {
				...
				/*for(int o : compile.addressTable.keySet()) {
					String[] arr = compile.addressTable.get(o);
						//COLLECT ADDRESS INFO AND SAVE IN MAP TO INSERT LATER
					if(arr[0].contains(address1) && arr[1].contains(address2) && arr[3].contains(location)) {
						exists = true;
						break;
						}
				}*/
				if(!tables.addressTable1.containsKey(address1+"-"+address2+"-"+location)) {					
					//addressID++;			
					int id = tables.addressTable1.size() + 1;
					...
					if( row.length>22)value[6]=row[22]; // SEVENTH  ARRAY POSITION IS CENSUS_BLOCK_GROUP
					InitiateDB.addressTable.put(id, value); // ADD TO A MAP IN THE COMPILE CLASS (key,value)
					tables.addressTable1.put(address1+"-"+address2+"-"+location,Integer.toString(id));
					writeFileName("D:\\add.txt",id + "," +address1+"-"+address2+"-"+location );
				}
