 ...
 * */
public class file2 { // DEFINE CLASS NAME
	public static void main(String[] args) {Config.setConfig("Crime");getURI();}
    ...
    public static boolean getURI() {
    	try(BufferedInputStream in = new BufferedInputStream(new URL(Config.inputURL).openStream());
    			FileOutputStream fileOutputStream = new FileOutputStream(Config.inputFile)) {
	...
	public static boolean collectCrimeEntities() { // THIS WILL BREAKDOWN OUR INPUT CSV
		try(BufferedReader br = new BufferedReader(new FileReader(Config.inputFile))) { // OPEN CRIME_INCIDENTS.CSV
			...
			file2.logMessage("Reading " + Config.inputFile);
			while((line=br.readLine()) != null) {
				...
				if(row.length > 6 && !DB.exists("incident", "case_number='"+row[1]+"' AND incident_id='"+row[0]+"';" )) {
					...
					if (Compile.incidentTable.size() == 5) {
						String sql =Compile.generateIncidentInsertStatement();
						DB.insert( sql);
						Compile.incidentTable = new ConcurrentHashMap<>();
					}
